...
/**
 * Class Potter_Plugin_Install_Helper
 */
class Potter_Plugin_Install_Helper
{
    /**
     ...
     *
     * @return Potter_Plugin_Install_Helper
     */
    public static function instance()
    {
        if (! isset(self::$instance) && ! (self::$instance instanceof Potter_Plugin_Install_Helper)) {
            self::$instance = new Potter_Plugin_Install_Helper();
        }
    ...
    public function get_button_html($slug, $settings = array())
    {
        ...
        switch ($state) {
            case 'install':
                $button .= '<a data-redirect="' . esc_url($redirect) . '" data-slug="' . esc_attr($slug) . '" class="install-now potter-install-plugin button  " href="' . esc_url($nonce) . '" data-name="' . esc_attr($slug) . '" aria-label="Install ' . esc_attr($slug) . '">' . __('Install and activate &raquo;', 'potter') . '</a>';
                break;

            case 'activate':
                $button .= '<a  data-redirect="' . esc_url($redirect) . '" data-slug="' . esc_attr($slug) . '" class="activate-now button button-primary" href="' . esc_url($nonce) . '" aria-label="Activate ' . esc_attr($slug) . '">' . esc_html__('Activate', 'potter') . '</a>';
                break;
    ...
    public function enqueue_scripts()
    {
        ...
        wp_enqueue_script('potter-plugin-install-helper', get_template_directory_uri() . '/inc/customizer/controls/ui/helper-plugin-install/helper-script.js', array( 'jquery' ), POTTER_VERSION, true);
        wp_localize_script(
            'potter-plugin-install-helper',
            'potter_plugin_helper',
            array(
